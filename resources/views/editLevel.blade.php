...
<div class="card">
    ...
    <div class="card-body">
            <form method="POST" action="/updateLevel/{{ $level->id }}">
                @csrf
                <div class="modal-body text-right font-weight-bold row" dir="rtl">
                    <div class="form-group col-lg-12">
                        ...
                        <input type="text" value="{{ $level->level }}" name="level" class="form-control text-right" required>
                    </div>
                        ...
                        <div class="row pages text-right  col-lg-12" align="right" dir="rtl">
                            @foreach ($page["subPage"] as $subPage)
                                <div class="col-lg-4" dir="rtl" style="font-size: .9375rem;">
                                    <input @if( in_array($subPage->id, $previousPermissions)) checked @endif value="{{ $subPage->id }}" name="page{{ $page->id }}[]" type="checkbox" class="form-check-input page{{ $page->id }}" style="width: 18px; height: 18px; border-radius: 2px;  border: solid #844fc1; border-width: 2px;">
                                    &nbsp;&nbsp;&nbsp;
                                    {{ $subPage->sub_page }}
                                </div>
                                <br>
