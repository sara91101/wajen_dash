...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="/updateProperty/{{ $property->id }}">
            @csrf
            <div class="modal-body text-right font-weight-bold row" dir="rtl">
                <div class="form-group col-lg-6">
                    ...
                    <select name="major_id" id="majors" class="form-select text-right" onchange="linkMinors(this)" required>
                        <option value="">-</option>
                        @foreach ($majors as $m)
                            <option value={{ $m->id }} @if($m->id == $property->major_id) selected @endif>{{ $m->major_ar }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-lg-6">
                    ...
                    <select name="minor_id" id="minors" class="form-select text-right" required>
                        <option value="">-</option>
                        @foreach ($minors as $n)
                            <option value={{ $n->id }} class="{{ $n->major_id }}"  @if($n->id == $property->minor_id) selected @endif>{{ $n->minor_ar }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-lg-6">
                    ...
                    <input type="text" name="property_ar" value="{{ $property->property_ar }} " class="form-control text-right" required>
                </div>
                <div class="form-group col-lg-6">
                    <label for="exampleInputUsername1" class="text-primary-purple">
                        الخاصية بالإنجليزية
                    </label>
                    <input type="text" name="property_en" value="{{ $property->property_en }} " class="form-control text-right">
                </div>
                ...
                <div class="row text-right" align="right" dir="rtl">
                    ...
                    @foreach ($operations as $p)
                        @php
                            $pos = in_array($p['name'], $compare);
                            $pos2 = in_array($p['name'], $used);
                        @endphp
                        @if(!is_null($p['name']) && str_contains($p['path'], "api/v"))
                            <div class="col-lg-4" dir="rtl" style="font-size: .9375rem;">
                                <input @if($pos) checked @endif value="{{ $p['name'] }}" name="proc[]" type="checkbox" class="form-check-input" style="width: 18px; height: 18px; border-radius: 2px;  border: solid #844fc1; border-width: 2px;">
                                &nbsp;&nbsp;&nbsp;
                                {{ $k }}. {{ $p["name"] }}
                            </div>
                            @php $k++; @endphp
