...
<div class="card">
    ...
    <div class="card-body">
            <form method="POST" action="/updatePackage/{{ $package->id }}">
                @csrf
                        ...
                        @if(count($m["minor"]) > 0)
                            ...
                            <div class="row majors text-right sys{{ $m->systm_id }}  col-lg-12" align="right" dir="rtl">
                                @foreach ($m["minor"] as $n)
                                    @php $check = $package["packageMinor"]->contains("id",$n->id); @endphp
                                    <div class="col-lg-4" dir="rtl" style="font-size: .9375rem;">
                                        <input @if($check) checked @endif value="{{ $n->id }}" name="major{{ $m->id }}[]" type="checkbox" class="form-check-input major{{ $m->id }}" style="width: 18px; height: 18px; border-radius: 2px;  border: solid #844fc1; border-width: 2px;">
                                        &nbsp;&nbsp;&nbsp;
                                        {{ $n->minor_ar }}
                                    </div>
                                @endforeach
